...
export default function Dashboard() {
  ...
  const exportReport = () => {
    const reportContent = `
CLOUVIE COMPREHENSIVE ANALYTICS REPORT
======================================
...
EXECUTIVE SUMMARY
-----------------
Total Products Analyzed: ${topMovers.length}
Opportunities Identified: 47
Potential Profit Uplift: $44,594.47
Average Price Elasticity: -1.200

KEY INSIGHTS
------------
• ${highPotentialProducts} products with high profit potential ($100+)
• ${lowMarginProducts} products with margins below 25%
• ${lowStockProducts} products with low stock levels

TOP OPPORTUNITIES
-----------------
${topMovers.slice(0, 5).map((product, index) => `
${index + 1}. ${product.name} (SKU: ${product.id})
   Current Price: $${product.currentPrice.toFixed(2)}
   Recommended Price: $${product.recommendedPrice.toFixed(2)}
   Potential Gain: $${product.profitGainLoss.toFixed(2)}
   Price Elasticity: ${product.bpe?.toFixed(2) || 'N/A'}
   Margin: ${product.margin?.toFixed(1) || 'N/A'}%
   Stock Level: ${product.stockLevel || 'N/A'} units
`).join('\n')}

PRIORITY ACTIONS
----------------
1. Immediate Price Optimization
   - ${highPotentialProducts} products ready for price adjustment
   - Estimated impact: $${topMovers.filter(p => p.profitGainLoss > 100).reduce((sum, p) => sum + p.profitGainLoss, 0).toFixed(2)}

2. Margin Improvement
   - ${lowMarginProducts} products need cost reduction or price increase
   - Target margin: 25%+

3. Inventory Management
   - ${lowStockProducts} products require reordering
...
Current Price: $${product.currentPrice.toFixed(2)}
Recommended Price: $${product.recommendedPrice.toFixed(2)}
Profit Impact: $${product.profitGainLoss.toFixed(2)}
Demand Change: ${product.demandChange?.toFixed(1) || 'N/A'}%
Elasticity: ${product.bpe?.toFixed(2) || 'N/A'}
Margin: ${product.margin?.toFixed(1) || 'N/A'}%
Stock: ${product.stockLevel || 'N/A'} units
---
`).join('\n')}

Generated by Clouvie AI Analytics Platform
Visit: https://clouvie.com
    `;
    ...
    a.href = url;
    a.download = `Clouvie_Analytics_Report_${new Date().toISOString().split('T')[0]}.txt`;
    ...
    <div className="p-4">
      ...
      <div className="mb-8 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        >
          ...
          <div className="text-sm text-white/80">Full analytics</div>
        </button>
        <button 
